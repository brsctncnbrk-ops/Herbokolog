...
async function main() {
  ...
  for (const p of PRODUCTS) {
    ...
    created.push({ id: prod.id, name: prod.name, category: prod.category });
  }
  console.log(`  ${created.length} ürün eklendi.`);

  // IngredientMap.
  await prisma.ingredientMap.createMany({ data: INGREDIENT_MAP });
  console.log(`  ${INGREDIENT_MAP.length} IngredientMap satırı eklendi.`);

  // Sponsorluklar: 1 aktif (saç bakımı/kepek) + 1 süresi geçmiş (cilt bakımı).
  ...
  if (sacProduct) {
    await prisma.sponsorship.create({
      data: {
        ...
        category: "sac_bakimi",
        subNeed: "kepek",
        startDate: new Date(now.getTime() - 7 * 86400000), // 7 gün önce
        endDate: new Date(now.getTime() + 30 * 86400000), // 30 gün sonra (AKTİF)
        adLabel: "Reklam",
      },
    });
  }
  if (ciltProduct) {
    await prisma.sponsorship.create({
      data: {
        ...
        subNeed: null,
        startDate: new Date(now.getTime() - 60 * 86400000), // 60 gün önce
        endDate: new Date(now.getTime() - 30 * 86400000), // 30 gün önce (SÜRESİ GEÇMİŞ)
        adLabel: "Reklam",
      },
    });
  }
  console.log("  2 sponsorluk eklendi (1 aktif, 1 süresi geçmiş).");
